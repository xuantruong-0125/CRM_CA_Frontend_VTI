...
export default function QuoteListPage() {
    ...
    const { data, loading, currentPage, setCurrentPage, itemsPerPage, setItemsPerPage, pagination, filters, applyFilter, resetFilter, remove } = useQuote();
    ...
    const handleSortToggle = (dir: "asc" | "desc") => {
        const next = { ...localFilters, sort: `id,${dir}` };
        setLocalFilters(next);
        applyFilter(next);
    };
    ...
    const currentSort = localFilters.sort ?? filters.sort ?? "id,desc";
    ...
    return (
        <div className={styles.container}>
            <div className={styles.pageHeader}>
                ...
                <button className={styles.btnPrimary} onClick={() => router.push("/crm/quotes/new")} title="Phím tắt: Alt + N">
            ...
            <div className={styles.toolbar}>
                ...
                <div className={styles.toolbarRight}>
                    ...
                    <button
                        className={`${styles.btnSort} ${currentSort === "id,asc" ? styles.btnSortActive : ""}`}
                    ...
                    <button
                        className={`${styles.btnSort} ${currentSort === "id,desc" ? styles.btnSortActive : ""}`}
                        ...
                        <div className={styles.modalBody}>
                            ...
                            <div className={styles.formGrid2}>
                                ...
                                <div className={styles.formGroup}>
                                    ...
                                    <input type="date" name="toDate" className={styles.formControl} value={localFilters.toDate ?? ""} onChange={handleFilterChange} />
                                </div>
                            </div>
                        </div>
                                    ...
                                    <tr key={quote.id}>
                                        ...
                                        <td className={styles.actionsCell} style={{ paddingRight: "16px" }}>
                                            <div className={styles.actions}>
                                                <button className={styles.btnIcon} onClick={() => router.push(`/crm/quotes/${quote.id}`)} title="Xem chi tiết"><Eye size={16} /></button>
                                                <button className={styles.btnIcon} onClick={() => router.push(`/crm/quotes/${quote.id}/edit`)} title="Chỉnh sửa"><Edit size={16} /></button>
                                                <button className={`${styles.btnIcon} ${styles.btnIconDanger}`} onClick={() => setDeleteId(quote.id)} title="Xóa"><Trash2 size={16} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                );
                ...
                {!loading && data.length > 0 && (
                    <div className={styles.pagination}>
                        <p className={styles.paginationInfo}>
                            Hiển thị{" "}
                            <strong>{(currentPage - 1) * itemsPerPage + 1}</strong>–<strong>{Math.min(currentPage * itemsPerPage, pagination.totalElements)}</strong>{" "}
                            trong <strong>{pagination.totalElements}</strong> báo giá
                        </p>
                        <div className={styles.paginationControls}>
                            ...
                            <button className={styles.pageBtn} disabled={currentPage === pagination.totalPages} onClick={() => setCurrentPage(currentPage + 1)}>
