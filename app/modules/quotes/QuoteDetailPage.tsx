...
import { ArrowLeft, Edit, Printer, Package, User, Calendar, Tag, FileText, TrendingDown, DollarSign, Send, CheckCircle } from "lucide-react";
...
export default function QuoteDetailPage() {
    ...
    const handleChangeStatus = async (newStatusId: number, msg: string) => {
        if (!quote) return;
    ...
    if (error || !quote) {
        return (
            <div style={{ textAlign: "center", padding: "3rem", color: "#ef4444" }}>
                ...
                <button className={styles.btnOutline} onClick={() => router.push("/quotes")}>
    ...
    return (
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <div className={styles.detailHeader}>
                <div>
                    <button className={styles.backLink} onClick={() => router.push("/quotes")}>
                ...
                <div className={styles.detailActions}>
                    ...
                    {/* Gửi khách — Sales + Admin, khi Bản nháp */}
                    {(isSales || isAdmin) && quote.statusId === 1 && (
                        <button className={styles.btnPrimary} disabled={statusUpdating}
                            onClick={() => handleChangeStatus(2, "Đã gửi báo giá cho khách")}>
                            <Send size={15} /> Gửi khách
                        </button>
                    )}

                    {/* Duyệt — Sales Manager + Admin, khi Đã gửi */}
                    {(isSaleManager || isAdmin) && quote.statusId === 2 && (
                        <button className={`${styles.btnPrimary} ${styles.btnSuccess}`} disabled={statusUpdating}
                            onClick={() => handleChangeStatus(3, "Đã duyệt báo giá")}>
                    ...
                    {(isSales || isAdmin) && (
                        <button className={styles.btnPrimary} onClick={() => router.push(`/quotes/${id}/edit`)}>
